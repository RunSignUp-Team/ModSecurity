    ...
    if (actions) {
        for (auto &a : *actions) {
            if (dynamic_cast<ActionTypeRuleMetaData *>(a.get())) {
                ...
                m_actionDisruptiveAction = std::dynamic_pointer_cast<ActionDisruptive>(a);
                continue;
            }
...
void RuleWithActions::addDefaultAction(std::shared_ptr<actions::Action> a) {
    ...
    if (std::dynamic_pointer_cast<ActionDisruptive>(a)) {
        m_defaultActions.m_actionDisruptiveAction = std::dynamic_pointer_cast<ActionDisruptive>(a);
        return;
    } 
...
void RuleWithActions::executeActionsAfterFullMatch(Transaction *trans) const {
    ...
    if (m_actionDisruptiveAction) {
        trans->messageGetLast()->setRule(this);
        fd(trans, m_actionDisruptiveAction);

    } else if (hasBlockAction() && m_defaultActions.m_actionDisruptiveAction) {
        trans->messageGetLast()->setRule(this);
        fd(trans, m_defaultActions.m_actionDisruptiveAction);
    }
}
